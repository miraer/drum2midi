...
""" % str(ROOT / "restem_batch_mode.ps1").replace("\\", "\\"), encoding="utf-8")
    ...
    # Windows only. The script loads UIAutomationClient.dll before -DefineOnly is
    # reached, so even PowerShell 7 on Linux (`pwsh`, which the Ubuntu CI image has)
    # stops at Add-Type and reports no cases. Measured on CI, not assumed. ReStem is a
    # Windows application, so there is no other platform for the guard to run on.
    if sys.platform != "win32":
        return
    shell = shutil.which("powershell") or shutil.which("pwsh")
    assert shell, "no PowerShell on a Windows machine; the guard was not exercised"
